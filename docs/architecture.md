...
**Consequence:** Every tool lives in `cmd/<tool>/<tool>.go` as its own package and exposes `func Run() int` that `main.go` calls via a registry map. No tool has its own `main()`. `main.go` calls `os.Exit(fn())`.
...
**Subcommands (kv, base):** manual switch on `os.Args[1]`, then a sub-`pflag.FlagSet` per subcommand. pflag handles flag parsing; you handle routing. No framework needed.
...
- `moniker` - adjective and noun wordlists (~50KB)
- `main.go` - manifest.json (tool registry), SKILLS.md (agent reference), skills/ filesystem
- `slug` - Unicode normalization tables (via golang.org/x/text)
...
**Decision:** `kv` uses `modernc.org/sqlite`. WAL mode enabled by default.
...
| `exit.go` | `Errorf`, `UsageErrorf`, `Warn` (for `Run()`); `Die`, `DieUsage` (`main()` only); exit constants 0/1/2 |
...
| `terminal.go` | `IsTerminal` - var so tests can override TTY detection |
...
## 10. Exit Codes - Strict, Never Change
...
## 11. Streaming - Explicit Flush, No Buffering
...
## 12. Release Pipeline - goreleaser
...
**Install script:** `hugo/static/install.sh` (served at `vrk.sh/install.sh`) detects platform, downloads the right binary, verifies SHA256, moves atomically. Idempotent - running it twice at the same version does nothing.

---

## 13. Website and Documentation - Hugo + Cloudflare Pages
...
**Content consistency rule:** tool reference pages live in `hugo/content/docs/<tool>.md`. Generated sections are below the `<!-- generated -->` comment. Never write the same docs in two places.
  ...
  deploy:
    ...
    steps:
      ...
      - name: Generate tool data
        run: go run ./internal/docgen/main/ --output hugo/data/tools/
      - name: Build Hugo
...
## 14. What Belongs in This Doc vs Elsewhere

| Question | Where it lives |
|----------|---------------|
| What flags exist and what do they mean? | `docs/flag-conventions.md` |
| How does each tool work? | `cmd/<tool>/<tool>.go` + `hugo/content/docs/<tool>.md` |
...
| What tools are planned but not built? | Project planning docs (outside repo) |
...
## 15. GitHub Organisation - `github.com/vrksh`
