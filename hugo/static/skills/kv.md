...
When to use: persist state between pipeline runs - cursors, counters, timestamps, cached results. SQLite-backed, supports namespaces, TTL, and atomic increment.
...
Anti-pattern:
- Don't use kv for large blobs. It's backed by SQLite with text values. Store file paths or URLs, not megabytes of content.
- Don't forget --ttl on cached values. Without TTL, keys persist forever and the database grows without bound.
