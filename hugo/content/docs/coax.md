...
You have a nightly pipeline that calls an LLM API 200 times. The API returns 429 (rate limited) on roughly 1 in 50 calls. You write a bash retry loop. It works until the API starts returning 500s on a bad deploy, and your retry loop hammers it 200 times with no backoff, making the outage worse.
