...
You're processing a list of URLs and need to group them by domain. You extract the hostname with cut -d'/' -f3. A URL has a port number: https://api.example.com:8080/path. Cut returns "api.example.com:8080" including the port. Your grouping breaks. Another URL has basic auth in it. Your regex didn't account for @ symbols.
