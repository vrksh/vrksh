...
[ "$tool_count" -eq 15 ] \
  && { echo "PASS: --manifest lists 15 tools"; PASS=$((PASS+1)); } \
  || { echo "FAIL: --manifest listed $tool_count tools (expected 15)"; FAIL=$((FAIL+1)); }
# each expected tool name must appear
for tool in jwt epoch uuid tok sse coax prompt kv chunk grab plain links validate mask emit; do
